...
GUI_Handler.prototype.displayBotMenu = function () {
    ...
    for (var accountIndex in accounts) {
        if (accounts.hasOwnProperty(accountIndex)) {
            tempList.push(
                accounts[accountIndex].getAccountName() +
                    "[{0}]".format(accounts[accountIndex].getDisplayName())
            );
        }
    }
                            ...
                            inquirer
                                ...
                                .then(function (result) {
                                    if (
                                        ...
                                        self.displayBotMenu();
                                        self.main.errorDebug(
                                            "One or more fields that are required are empty."
                                        );
                                    } else {
                                        self.main
                                            .randomBot({})
                                            .createAccount(
                                                ...
                                                function (eresult, steamid) {
                                                    if (eresult != EResult.OK) {
                                                        self.main.errorDebug(
                                                            "Failed to create account due to error: " +
                                                                EResult[eresult]
                                                        );
                                                        self.displayBotMenu();
                                                    } else {
                                                        self.main.registerAccount(
                                                            result.username,
                                                            result.password,
                                                            [],
                                                            function (err) {
                                                                if (err)
                                                                    self.main.errorDebug(
                                                                        "Invalid username/password, either edit data file or try to register again.."
                                                                    );
                                                                else
                                                                    self.main.infoDebug(
                                                                        "Successfully added new account to node-steam-bot-manager."
                                                                    );
                                                                self.displayBotMenu();
                                                            }
                                                        );
                            ...
                            accountQuestions = [
                                {
                                    ...
                                    name: "username",
                                    message:
                                        "What's the bot username? [Required]",
                                },
                                {
                                    type: "password",
                                    name: "password",
                                    message:
                                        "What's the bot password? [Required]",
                                },
                                {
                                    type: "input",
                                    name: "shared_secret",
                                    message:
                                        "What's the shared_secret? (Optional)",
                                },
                                {
                                    type: "input",
                                    name: "identity_secret",
                                    message:
                                        "What's the identity_secret? (Optional)",
                                },
                                {
                                    type: "input",
                                    name: "revocation",
                                    message:
                                        "What's the revocation code? (Optional)",
                                },
                            ];
...
GUI_Handler.prototype.processChat = function (botAccount, target) {
    ...
    inquirer.prompt(chatMessage).then(function (result) {
        if (
            result.message.toLowerCase() == "quit" ||
            result.message.toLowerCase() == "exit"
        ) {
            botAccount.setChatting(null);
            self.displayMenu(botAccount);
        } else {
            botAccount.Friends.sendMessage(
                target,
                result.message,
                function (err) {
            ...
            inquirer.prompt(tradeMenu).then(function (result) {
                ...
                switch (menuEntry) {
                    ...
                    default:
                        ...
                        botAccount.Trade.createOffer(
                            partner.accountSid,
                            function (err, currentOffer) {
                                if (err) {
                                    self.main.errorDebug(
                                        "Failed to create offer due to " + err
                                    );
                                            ...
                                            function (
                                                ...
                                                if (err) {
                                                    ...
                                                    if (
                                                        ...
                                                        self.main.infoDebug(
                                                            "Other user has no items in inventory. Redirecting to menu..."
                                                        );
                                                        self.initTradeMenu(
                                                            botAccount
                                                        );
                                                    ...
                                                    var tradeMenu = [
                                                        {
                                                            ...
                                                            name: "tradeOption",
                                                            message:
                                                                "What would you like to take? ('Enter' to send trade)",
                                                            choices: nameList,
                                                        },
                                                    ];
                                                    inquirer
                                                        ...
                                                        .then(function (
                                                            ...
                                                            if (
                                                                ...
                                                                for (var itemNameIndex in result.tradeOption) {
                                                                    if (
                                                                        result.tradeOption.hasOwnProperty(
                                                                            itemNameIndex
                                                                        )
                                                                    ) {
                                                                        var itemName =
                                                                ...
                                                                currentOffer.setMessage(
                                                                    "Manual offer triggered by Bot Manager."
                                                                );
                                                                currentOffer.send(
                                                                    function (
                                                                        err,
                                                                        ...
                                                                        if (
                                                                            ...
                                                                            botAccount.Trade.confirmOutstandingTrades(
                                                                                function (
                                                                                    err,
                                                                                    confirmedTrades
                                                                                ) {
                                                                                    self.main.infoDebug(
                                                                                        "Sent trade offer to %s." +
                                                                                            partner.username
                                                                                    );
                                                                                    self.displayMenu(
                                                                                        botAccount
                                        ...
                                        botAccount.getInventory(
                                            ...
                                            function (
                                                ...
                                                if (
                                                    ...
                                                    self.main.infoDebug(
                                                        "Bot has no items in inventory. Redirecting to menu..."
                                                    );
                                                    self.initTradeMenu(
                                                        botAccount
                                                    );
                                                ...
                                                var tradeMenu = [
                                                    {
                                                        ...
                                                        name: "tradeOption",
                                                        message:
                                                            "What would you like to offer? ('Enter' to send trade)",
                                                        choices: nameList,
                                                    },
                                                ];
                                                inquirer
                                                                    ...
                                                                    if (err) {
                                                                        ...
                                                                        botAccount.Trade.confirmOutstandingTrades(
                                                                            function (
                                                                                ...
                                                                                self.main.infoDebug(
                                                                                    "Sent trade offer to %s, and confirmed %s.",
                                                                                    partner.username,
...
GUI_Handler.prototype.initTradeMenu = function (botAccount) {
    var self = this;
    var tradeOptions = ["Request Items", "Give Items", "Back"];
...
GUI_Handler.prototype.displayMenu = function (botAccount) {
    ...
    if (self.main.config.api_key == undefined) {
        ...
        menuOptions[1] =
            "Send trade offer [Disabled - missing 'api_key' in config]";
    }
        ...
        switch (menuEntry) {
                    ...
                    botAccount.Friends.getFriends(function (err, friendsList) {
                        if (err) {
                            ...
                            inquirer.prompt(chatMenu).then(function (result) {
                                var menuEntry = nameList.indexOf(
                                    result.chatOption
                                );
                                // We will open chat with...
                                switch (menuEntry) {
                                    ...
                                    default:
                                        ...
                                        botAccount.setChatting({
                                            username:
                                                friendsList[menuEntry].username,
                                            sid: friendsList[menuEntry]
                                                .accountSid,
                                        });
                                        self.processChat(
                                            botAccount,
                                            friendsList[menuEntry].accountSid
                                        );
                                        break;
                                }
                            });
            ...
            case 3:
                ...
                if (!botAccount.loggedIn) {
                    self.main.infoDebug(
                        "Trying to authenticate into {0}".format(
                            botAccount.getAccountName()
                        )
                    );
                                ...
                                var authenticator = [
                                    {
                                        ...
                                        name: "code",
                                        message:
                                            "Enter the authenticator code sent to your " +
                                            err["emaildomain"] +
                                            " email account for " +
                                            botAccount.username +
                                            ": ",
            ...
            case 5:
                ...
                authOptions.push(
                    (botAccount.Auth.has_shared_secret() ? "[ON]" : "[OFF]") +
                        " 2-Factor Authentication"
                );
                authOptions.push(
                    (!botAccount.Auth.has_shared_secret() ? "[Disabled]" : "") +
                        "Generate 2-Factor Authentication code"
                );
                ...
                var authMenu = [
                    {
                        ...
                        name: "authOption",
                        message:
                            "Choose the authentication option you would like to activate.",
                        choices: authOptions,
                    },
                ];
                    ...
                    switch (optionIndex) {
                        case 0:
                            var questions = [
                                ...
                                },
                                {
                                    ...
                                    default: true,
                                    message:
                                        "Give default prefix of '{0}'?".format(
                                            self.main.config.bot_prefix
                                        ),
                                },
                            ];

                            inquirer.prompt(questions).then(function (result) {
                                botAccount.Profile.changeDisplayName(
                                    result.newName,
                                    result.prefix
                                        ? self.main.config.bot_prefix
                                        : undefined,
                                    function (err) {
                                        ...
                                        if (err) {
                                            self.main.errorDebug(
                                                "Failed to change name. Error: {0}".format(
                                                    err
                                                )
                                            );
                                        } else {
                                            self.main.infoDebug(
                                                "Successfully changed display name"
                                            );
                                        }
                                    }
                                );
                                ...
                                var questions = [
                                    {
                                        ...
                                        default: false,
                                        message:
                                            "Are you sure you want to disable 2-Factor Authentication on '" +
                                            botAccount.username +
                                            "' account?",
                                    },
                                ];
                                inquirer
                                    ...
                                    .then(function (answers) {
                                        if (answers.askDelete) {
                                            botAccount.Auth.disableTwoFactor(
                                                function (err) {
                                                    self.displayMenu(
                                                        botAccount
                                                    );
                                                    if (err) {
                                                        self.main.errorDebug(
                                                            "Encountered error while disabling 2-Factor Authentication: " +
                                                                err.toString()
                                                        );
                                                    } else {
                                                        self.main.infoDebug(
                                                            "2-Factor Authentication has been successfully disabled."
                                                        );
                                                    }
                                                }
                                            );
                        ...
                        case 3:
                            ...
                            if (botAccount.Auth.has_shared_secret()) {
                                // Send the auth key.
                                self.main.infoDebug(
                                    "\nYour 2-Factor Authentication code for {0} is {1}".format(
                                        botAccount.getAccountName(),
                                        botAccount.Auth.generateMobileAuthenticationCode()
                                    )
                                );
                            } else {
                                // Auth not enabled?
                                self.main.errorDebug(
                                    "2-Factor Authentication is not enabled. Check your email."
                                );
                            }
                            break;
                        default:
                ...
                var questions = [
                    {
                        ...
                        name: "askDelete",
                        message:
                            "Are you sure you want to delete '{0}' account?".format(
                                botAccountName
                            ),
                    },
                ];
                    ...
                    if (answers.askDelete) {
                        self.main.deleteAccount(botAccount, function (err) {
                            if (err) {
                                // Failed...
                                self.main.errorDebug(
                                    "Failed to delete account data due to " +
                                        err
                                );
                            } else {
                                self.main.infoDebug(
                                    "Successfully deleted {0} from node-steam-bot-manager. A backup of account info was saved under 'deleted'.".format(
                                        botAccountName
                                    )
                                );
                                self.displayBotMenu();
        ...
        if (hasPhone) {
            botAccount.Auth.enableTwoFactor(function (err, response) {
                if (status == 84) {
                    ...
                    self.main.infoDebug(
                        "Please wait 5 seconds to continue... Possibly blocked by Steam for sending out too many SMS's. Retry in 24 hours, please."
                    );
                    setTimeout(function () {
                        self.enableTwoFactor(botAccount);
                    }, 5000);
                    ...
                    self.main.infoDebug(
                        "Make sure to save the following code saved somewhere secure: {0}".format(
                    ...
                    var questions = [
                        {
                            ...
                            name: "code",
                            message:
                                "Enter the code texted to the phone number associated (-{0}) to the account: ".format(
                                    lastDigits
                                ),
                        },
                    ];
                    ...
                    inquirer.prompt(questions).then(function (result) {
                        if (result.code) {
                            ...
                            botAccount.Auth.finalizeTwoFactor(
                                steamCode,
                                function (err, accountDetails) {
                                    if (err) {
                                        self.main.errorDebug(
                                            "Failed to enable 2 factor auth - " +
                                                err
                                        );
                                    } else {
                                        self.main.AccountsManager.saveAccount(
                                            accountDetails,
                                            function (err) {
                                                if (err) {
                                                    self.main.errorDebug(
                                                        "Failed to save accounts during 2 factor auth enable - " +
                                                            err
                                                    );
                                                }
                                                self.displayBotMenu();
                    ...
                    self.main.errorDebug(
                        "Error encountered while trying to enable two-factor-authentication, error code: " +
                            response
                    );
                    self.displayBotMenu();
                }
            ...
            var questions = [
                {
                    ...
                    message:
                        "A phone number is required to activate 2-factor-authentication. Would you like to set your phone number?",
                    default: false,
                },
            ];
            ...
            inquirer.prompt(questions).then(function (result) {
                if (result.confirmAddition) {
                    var questions = [
                        {
                            ...
                            name: "phoneNumber",
                            message:
                                "Enter the number you would like to link to the account (ex. +18885550123)",
                    ...
                    inquirer.prompt(questions).then(function (result) {
                        botAccount.addPhoneNumber(
                            result.phoneNumber,
                            function (err) {
                                if (err) {
                                    self.main.errorDebug(
                                        "Error while adding phone number: " +
                                            err
                                    );
                                    self.displayMenu(botAccount);
                                } else {
                                    var questions = [
                                        {
                                            ...
                                            name: "code",
                                            message:
                                                "Enter the code sent to your phone number at " +
                                                result.phoneNumber,
                                        },
                                    ];
                                    ...
                                    inquirer
                                        .prompt(questions)
                                        .then(function (result) {
                                            botAccount.verifyPhoneNumber(
                                                result.code,
                                                function (err) {
                                                    if (err) {
                                                        self.main.errorDebug(
                                                            "Error while verifying phone number: " +
                                                                err
                                                        );
                                                        self.displayMenu(
                                                            botAccount
