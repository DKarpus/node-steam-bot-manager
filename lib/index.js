...
BotManager.prototype.deleteAccount = function (botAccount, callbackErrorOnly) {
    ...
    var botIndex = self.ActiveBotAccounts.indexOf(botAccount);
    self.ActiveBotAccounts.splice(botIndex, 1);
...
BotManager.prototype.findBot = function (keyData, callback, filters) {
    ...
    if (keyData != null) {
        ...
        for (let index in botAccounts) {
            ...
            if (botAccount.hasOwnProperty("SteamID") && botAccount.SteamID) {
        ...
        botAccount.community.loggedIn(function (err, loggedIn) {
            if (!loggedIn) {
                ...
                if (filters.indexOf('logInIfNot') != -1){
                    if (self.botsLoggingIn.indexOf(botAccount.getAccountName()) == -1) {
                        if (err) self.errorDebug("Attempting to login from findBot into %s", botAccount.getAccountName());
                        self.botsLoggingIn.push(botAccount.getAccountName());
                        botAccount.Auth.loginAccount(function (err) {
                            if (err) self.errorDebug("Encountered error while logging in... %s", err)
                            self.botsLoggingIn.splice(self.botsLoggingIn.indexOf(botAccount.getAccountName()), 1);
                        });
                    } else {
                        self.logDebug("Waiting on login to process for %s.", botAccount.getAccountName());
                    }
                }
                callback(false);
            } else {
...
BotManager.prototype.registerAccount = function (username, password, options, callback) {
    let self = this;
    let config = self.ConfigManager.getConfig();
    let botAccount = new BotAccount(username, password, options, config);


    botAccount.on('sentOfferChanged', function (offer, oldState) {
        self.emit('sentOfferChanged', botAccount, offer, oldState);
    });
    botAccount.on('recreate', function (callbackFromRecreate) {
        delete self.ActiveBotAccounts[self.ActiveBotAccounts.indexOf(botAccount)];
        self.registerAccount(username, password, options, callbackFromRecreate);
    });
    ...
    botAccount.on('chatLoggedOn', function () {
        self.emit('chatLoggedOn', botAccount);
    });
    ...
    self.emit('loadedAccount', username, password, botAccount);
...
BotManager.prototype.chooseRandomBot = function () {
    let self = this;
    let randomBotIndex = Math.floor((Math.random() * self.getAccounts().length) % 1 == 0 && (Math.random() * self.getAccounts().length) > 0 ? self.getAccounts().length - 1 : (Math.random() * self.getAccounts().length));
...
BotManager.prototype.randomBot = function (filters) {
    ...
    if (filters.length == 0) {
        ...
        let botsMeetingFilters = [];

        for (let botIndex in self.getAccounts()) {
