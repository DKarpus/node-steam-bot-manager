...
function AccountsManager(fileManager) {
    var self = this;
    ...
    self.fileManager.createFolderIfNotExist("data", function(err){
        if (err)
            self.emit("error", "Failed to create 'data' folder under 'config'.");
        else
            self.emit("debug", "Created 'data' folder if it does not exist.");
...
AccountsManager.prototype.loadAccounts = function (callback) {
    var self = this;
    var accounts_temp = [];
    self.emit("debug", "Loading accounts - ", self.fileManager.exists("accounts.json"));
    ...
    if (self.fileManager.exists("data")) {
                ...
                for (var fileNameIndex in fileList) {
                    if (fileList.hasOwnProperty(fileNameIndex)) {
                        ...
                        self.fileManager.getFile(fileList[fileNameIndex], self.defaultAccount, function (err, accountDetails) {
                            if (err) {
                                self.emit("error", "Failed to load %j due to %j", fileList[fileNameIndex], err);
                            }
        ...
        if (self.fileManager.exists("accounts.json")) {
            self.fileManager.createFolderIfNotExist("data", function(err){
                if (err)
                    self.emit("error", "Failed to create folder named 'data' under config. Error encountered: " + err);
                else {
                    self.emit("debug", "Loading accounts via old method - Converting to new version...");
                            ...
                            for (var accountIndex in accounts) {
                                if (accounts.hasOwnProperty(accountIndex)) {
                                    ...
                                    self.saveAccount(accounts[accountIndex], function (err) {
                                        if (err)
                                            self.emit("error", "Failed to save into new format during conversion...");
                                    })
                                }
                            }
                            return callback(null, accounts_temp.slice());
                        } catch (e) {
                            self.emit("error", "Failed to read account data - check file for any malformation using a JSON parser - ", e);
                            return callback(null, []);
                        }
                    });
                }
