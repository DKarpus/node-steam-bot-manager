...
FileManager.prototype.getFileList = async function (pattern, callback) {
    if (!pattern || typeof callback !== 'function') {
        return callback(new Error('Invalid parameters'));
    }

    const self = this;
    const searchPath = path.join(self.fileManagerPath, pattern);

    try {
        const files = await glob(searchPath);
        const cleanFiles = files.map(file => 
            file.replace(self.fileManagerPath, '')
        );
